...
function PromoForm({ promo, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (promo) {
      setFormData({
        ...
        applicable_to_category: promo.applicable_to_category || 'ALL',
      })
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (promo) {
        await offersService.updatePromo(promo.id, data)
      } else {
        await offersService.createPromo(data)
      }
      onSuccess()
    } catch (error) {
      console.error('Error saving promo:', error)
      const errorMessage = error.response?.data?.message || 'Failed to save promo'
      alert(errorMessage)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ marginTop: 0 }}>{promo ? 'Edit Promo' : 'Create Promo'}</h2>
